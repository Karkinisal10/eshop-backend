...
class productController{
    ...
    add_product = async(req,res) => {
        ...
        form.parse(req, async(err, field, files) => {
            let {name = '', category = '',description = '', stock = '',price = '', discount = 0,shopName = '',brand = ''} = field;
            let {images} = files;
            ...
            try {
                ...
                // Resolve shopName from seller profile if not provided
                if (!shopName) {
                    const seller = await sellerModel.findById(id)
                    if (seller) {
                        shopName = (seller.shopInfo && seller.shopInfo.shopName) ? seller.shopInfo.shopName : (seller.name || 'Shop')
                    }
                }
