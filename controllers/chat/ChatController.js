...
class ChatController {
    add_customer_friend = async (req, res) => {
        ...
        try {
            if (sellerId !== '') {
                ...
                // Ensure both participants have chat docs
                    ...
                    await sellerCustomerModel.updateOne(
                        { myId: userId },
                        {
                            $push: {
                                myFriends: {
                                    fdId: sellerId,
                                    name: seller?.shopInfo?.shopName || seller?.name || 'Shop',
                                    image: seller?.image || ''
                                }
                            }
                        }
    ...
    customer_message_add = async (req, res) => {
        ...
        try {
            ...
            // Fallback sender name from customer record if not provided
            let senderName = name
            if (!senderName) {
    ...
    seller_message_add = async (req, res) => {
        ...
        try {
            await sellerCustomerModel.updateOne(
                { myId: senderId },
                { $setOnInsert: { myId: senderId, myFriends: [] } },
                { upsert: true }
            )
            await sellerCustomerModel.updateOne(
                { myId: receverId },
                { $setOnInsert: { myId: receverId, myFriends: [] } },
                { upsert: true }
            )

            // Fallback sender name from seller record if not provided
            let senderName = name
            if (!senderName) {
                const seller = await sellerModel.findById(senderId)
                senderName = seller?.shopInfo?.shopName || seller?.name || 'Seller'
            }
